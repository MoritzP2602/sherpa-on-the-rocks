...
if [ -z "$PROF2_IPOL" ]; then
  echo "ERROR: No prof2-ipol provided as first argument!" >&2
  exit 1
fi
PROF2_IPOL="$(realpath "$PROF2_IPOL")"
if [ ! -x "$PROF2_IPOL" ]; then
  echo "ERROR: prof2-ipol not found or not executable: $PROF2_IPOL" >&2
  exit 1
fi
if [ -n "$RIVET_ENV" ]; then
  if [ ! -f "$RIVET_ENV" ]; then
...
term_handler() {
  echo "Received termination signal. Forwarding SIGINT to prof2-ipol..."

  if [ -n "$prof2_ipol_pid" ]; then
    kill -INT "$prof2_ipol_pid" 2>/dev/null
    wait "$prof2_ipol_pid" 2>/dev/null || true
  fi
  echo ""
  print_end_time
  echo ""
...
# Unlike app-build, prof2-ipol names its output as the SECOND POSITIONAL
# argument rather than with -o:  prof2-ipol <runsdir> [<ipolfile>] [opts]
# prepare_prof2-ipols.sh always writes the lines in that order.
if [ "${#IPOL_ARGS[@]}" -ge 2 ] && [ "${IPOL_ARGS[1]#-}" = "${IPOL_ARGS[1]}" ]; then
  OUTPUT_TARGET="${IPOL_ARGS[1]}"
else
  OUTPUT_TARGET="ipol.dat"
fi

CMD=("$PROF2_IPOL" "${IPOL_ARGS[@]}")
# prof2-ipol is threaded rather than MPI-parallel: -j takes the thread count.
if [ "$NPROC" -gt 1 ]; then
  CMD+=(-j "$NPROC")
fi

echo "PROF2_IPOL : $PROF2_IPOL"
...
echo "PROF2-IPOL will be terminated after $TIMEOUT seconds (2 minutes before wall time limit)!"
echo ""

echo "Starting prof2-ipol..."
echo ""
ipol_start_epoch=$(date +%s)

timeout --foreground -s INT -k 60 "$TIMEOUT" "${CMD[@]}" &
prof2_ipol_pid=$!
exit_code=0
wait "$prof2_ipol_pid" || exit_code=$?

if [ $exit_code -ne 0 ]; then
  ipol_elapsed=$(( $(date +%s) - ipol_start_epoch ))
  echo ""
  if { [ $exit_code -eq 124 ] || [ $exit_code -eq 137 ] || [ $exit_code -eq 130 ]; } && [ $ipol_elapsed -ge $TIMEOUT ]; then
    echo "prof2-ipol was terminated after reaching the time limit of $TIMEOUT seconds!"
    echo ""
    print_end_time
    echo ""
    {
    ...
    echo "prof2-ipol failed with exit code $exit_reason after $ipol_elapsed seconds"
    echo ""
    print_end_time
    echo ""
    {
...
echo ""
echo "prof2-ipol completed successfully."
echo ""
print_end_time
echo ""
echo "Copying output files back to shared filesystem..."
