...
emit('RIVET_ENV_SCRIPT', s.get('rivet_env_script', ''))
emit('SHERPA_ON_THE_ROCKS_DIR', s.get('sherpa_on_the_rocks_dir', ''))
emit('APP_TOOLS_INSTALLATION', s.get('app_tools_installation', ''))
emit('APPRENTICE_INSTALLATION', s.get('apprentice_installation', ''))
emit('SHERPA_BINARY', s.get('sherpa_binary', ''))
emit('MPI_MODULE', s.get('mpi_module', 'mpi/openmpi-x86_64'))
...
emit('ORDER', s['surrogate_order'])
emit('ORDER_SAFE', s['surrogate_order_safe'])
emit('N_GRID', s['n_grid'])
emit('MERGE_MODE', s['merge_mode'])
emit('COMBINE_MODE', s.get('combine_mode', 'weighted'))
emit('MERGED_DIR', s.get('merged_dir', ''))
