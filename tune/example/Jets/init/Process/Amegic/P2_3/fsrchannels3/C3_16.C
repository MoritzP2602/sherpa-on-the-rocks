...
extern "C" Single_Channel * Getter_C3_16(int nin,int nout,Flavour* fl,Integration_Info * const info) {
  return new C3_16(nin,nout,fl,info);
}

void C3_16::GeneratePoint(Vec4D * p,Cut_Data * cuts,double * _ran)
{
  ...
  Vec4D  p1_3 = p[1]-p[3];
  CE.TChannelMomenta(p[0],p1_3,p[2],p[4],s2,s4,0.,m_alpha,m_ctmax,m_ctmin,ran[3],ran[4]);
}

void C3_16::GenerateWeight(Vec4D* p,Cut_Data * cuts)
{
  double wt = 1.;
  Vec4D p234=p[0]+p[1];
  double s234_max = p234.Abs2();
  ...
  if (m_kTC_0__1_3_2_4.Weight()==ATOOLS::UNDEFINED_WEIGHT)
    m_kTC_0__1_3_2_4<<CE.TChannelWeight(p[0],p1_3,p[2],p[4],0.,m_alpha,m_ctmax,m_ctmin,m_kTC_0__1_3_2_4[0],m_kTC_0__1_3_2_4[1]);
...
C3_16::C3_16(int nin,int nout,Flavour* fl,Integration_Info * const info)
       : Single_Channel(nin,nout,fl)
{
  Settings& s = Settings::GetMainSettings();
  m_name = std::string("C3_16");
  m_rannum = 5;
...
void C3_16::ISRInfo(int & type,double & mass,double & width)
{
  type  = 2;
  mass  = 0;
  width = 0.;
...
void C3_16::AddPoint(double Value)
{
  Single_Channel::AddPoint(Value);
  p_vegas->AddPoint(Value,p_rans);
}
std::string C3_16::ChID()
{
  return std::string("CG2$MTH_24$TC_0__1_3_2_4$TC_0__1__24_3$ZS_0$");
}
