...
PHASE_KEY="P10_dir${DIR_INDEX}"
TAG="dir${DIR_INDEX}"
log_msg "10" "$TAG" "Started."
...
require_inputs "10" "$TAG" "${REQUIRED_INPUTS[@]}"
...
run_cmd "10" "$TAG" bash "$SHERPA_ON_THE_ROCKS_DIR/prepare_runs.sh" validation "$N_VAL_SUBRUNS" \
        --quiet --absolute --init="$INPUT_DIR/init"

if [[ ! -f runs.txt ]]; then
  log_msg "10" "$TAG" "Failed: prepare_runs.sh did not create runs.txt."
  exit 1
fi

log_msg "10" "$TAG" "Completed successfully."
