...
remove_merged_subdirs() {
    local -A seen=()
    local file subdir
    for file in "$@"; do
        subdir=$(dirname "$file")
        [ -n "${seen[$subdir]}" ] && continue
        seen["$subdir"]=1
    ...
    if [ "${#FILES[@]}" -eq 0 ]; then
        ...
        if [ "$CHUNK_SIZE" -gt 0 ]; then
            ...
            if [ "$REMOVE_SUBDIRS" = true ] && [ -f "$OUTFILE" ] && [ -z "$OUTPUT_DIR" ]; then
                [ "$QUIET" = false ] && echo "Removing merged subdirectories in $dir..."
                remove_merged_subdirs "${FILES[@]}"
            fi
        fi
    fi
}
...
merge_flat() {
    ...
    if [ "${#FILES[@]}" -eq 0 ]; then
        ...
        if [ "$CHUNK_SIZE" -gt 0 ]; then
            ...
            rivet_merge "${FILES[@]}" "$OUTFILE"
            if [ "$REMOVE_SUBDIRS" = true ] && [ -f "$OUTFILE" ] && [ -z "$OUTPUT_DIR" ]; then
                [ "$QUIET" = false ] && echo "Removing merged subdirectories in $PREFIX..."
                remove_merged_subdirs "${FILES[@]}"
            fi
        fi
    fi
...
export -f remove_merged_subdirs
...
if ! command -v rivet-merge >/dev/null 2>&1; then
    echo "Error: rivet-merge command not found. Please install RIVET or adjust the script to point to the correct path!" >&2
    exit 1
fi
